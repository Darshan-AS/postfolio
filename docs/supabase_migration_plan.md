...
5. **Data Persistence Architecture**: **Database Views (Reads) + Strongly-Typed RPC Stored Procedures (Writes) + Base-Table CDC (Realtime)**. 
   * *Reads & Realtime*: Relational joins across multiple tables (`customers`, `account_identities`, `savings_accounts`, `nominees`) are pre-joined into server-side Postgres Views (`customer_details_view`, `one_time_deposit_details_view`, `recurring_deposit_details_view`). Because Postgres Views do not produce Write-Ahead Log (WAL) events for Supabase Realtime WebSockets, repositories stream physical base tables (`customers` or `account_identities`) configured with `REPLICA IDENTITY FULL` to receive CDC event triggers, and map through `asyncMap` to fetch pre-joined JSON payloads from the respective view. Base tables are added to the `supabase_realtime` publication.
   * *Writes*: Relational persistence operations are wrapped inside server-side PL/pgSQL Stored Procedures (`save_customer_with_sb_account`, `save_one_time_deposit`, `save_recurring_deposit`) accepting explicit, strongly-typed arguments. The Flutter repository invokes them using `supabase.rpc()`.
   * *Why*: Guarantees **100% atomic database transactions** in a single HTTP roundtrip over mobile network connections (eliminating orphaned/corrupted database records if a connection drops) while ensuring instant real-time UI updates without manual screen refreshes.
